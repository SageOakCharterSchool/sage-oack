...
{
    use HasFactory;
    protected $tablesUsed2, $formulasUsed2;
    ...
    protected function columnACADEquivalences($returnColor = null)
    {
        $equivalences = [];
        $equivalencesColor = [];
        $rows = Equivalences::get();
        foreach ($rows as $row) {
            $equivalences[$row->equivalence] = $row->value;
            $equivalencesColor[$row->equivalence] = $row->color;
        }

        if (!$returnColor) {
            return $equivalences;
        }
        return $equivalencesColor;
    }
    ...
    protected function generateCycleTables($cycle,$fields)
    {
        //dd($fields);
        ...
        $result = Schema::create($tempTableName, function (Blueprint $table) use ($fields) {
            ...
            foreach ($fields as $field) {
                ...
                if ($field->table_source) {
                    ...
                    $this->tablesUsed2[$field->table_source][] = $fieldInfo[1];
                    //dd($tablesUsed,$field);
                } else if ($field->formula_id) {
                    $this->formulasUsed2[$field->formula_id] = $field->formula_id;
                }
            }
            $table->timestamps();
        ...
        return [
            'formulasUsed' => $this->formulasUsed2,
            'tablesUsed' => $this->tablesUsed2
        ];
    }
}
